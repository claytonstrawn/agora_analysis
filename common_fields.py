...
PartType_StarBeforeFiltered_to_use_dict = {'art':'all','ramses':'all','enzo':'all','gadget':'PartType4',
                       'gear':'PartType1','changa':'all','gizmo':'PartType4'}
...
StarMassType_to_use_dict = {'art':'particle_mass','ramses':'particle_mass','enzo':'particle_mass','gadget':'Mass',
                       'gear':'Masses','changa':'Mass','gizmo':'Masses'}
...
GasMetallicityType_to_use_dict = {'art':'metallicity','ramses':'metallicity',\
                              'enzo':'metallicity','gadget':'Metallicity',\
                              'gear':'metallicity','changa':'metallicity','gizmo':'metallicity'}
...
def add_metallicity_fields(snap):
    pf = snap.ds
    code = snap.code
    GasMetallicityType_to_use = GasMetallicityType_to_use_dict[code]
    if code == 'art': 
        ...
        def art_star_metallicity(field, data):
            return (data[PartType_Star_to_use, "particle_metallicity1"]+\
                    data[PartType_Star_to_use, "particle_metallicity2"])
        ...
        def gadget_metallicity(field, data):
            return data[(PartType_Gas_to_use, GasMetallicityType_to_use)]+\
                    YTArray(1e-4*0.02041,'')
        pf.add_field(('gas', 'agora_metallicity'), 
                     function=gas_metallicity, 
                     force_override=True,
                     take_log=True, 
        ...
        def gear_metallicity(field, data):
            if len(data[PartType_Gas_to_use, 'Metals'].shape) == 1:
                return data[PartType_Gas_to_use, "Metals"].in_units("")
            else:
...
def add_resolution_fields(snap):
    pf = snap.ds
    code = snap.code
    def cell_size(field,data):
        return (data[("index", "cell_volume")].in_units('pc**3'))**(1/3.)
    pf.add_field(("index", "cell_size"), function=cell_size, \
                 units='pc', display_name="Resolution $\Delta$ x", take_log=True )
    def inverse_cell_volume_squared(field,data):
        return data[("index", "cell_volume")]**-2
    pf.add_field(("index", "cell_volume_inv2"), function=inverse_cell_volume_squared, \
                 units='code_length**(-6)', display_name="Inv2CellVolumeCode", take_log=True)
    if code in particle_codes:
        def particle_size(field, data):
            return (data[('gas', 'agora_mass')]/data[('gas', "agora_density")])**(1./3.)
        pf.add_field(('gas', "agora_particle_size"), 
                     function=particle_size, 
                     units="pc", 
